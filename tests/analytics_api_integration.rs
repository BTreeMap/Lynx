...
use lynx::config::{Config, AuthConfig, AuthMode};
...
async fn test_analytics_aggregate_api_endpoint() {
    ...
    let us_agg = aggregates.iter()
        .find(|a| a["dimension"] == "US")
        .unwrap();
    assert_eq!(us_agg["visit_count"], 13);
    
    // Verify GB has 5 visits
    let gb_agg = aggregates.iter()
        .find(|a| a["dimension"] == "GB")
        .unwrap();
...
async fn test_analytics_aggregate_with_aggregator_realtime() {
    ...
    let time_bucket = 1698768000;
    let records = vec![
        ("realtime".to_string(), time_bucket, Some("US".to_string()), Some("CA".to_string()), Some("SF".to_string()), Some(15169), 4, 10),
    ];
    storage.upsert_analytics_batch(records).await.unwrap();
...
async fn test_analytics_aggregate_with_time_range() {
    ...
    let records = vec![
        ("timed".to_string(), 1000, Some("US".to_string()), Some("CA".to_string()), None, None, 4, 5),
        ("timed".to_string(), 2000, Some("GB".to_string()), Some("England".to_string()), None, None, 4, 3),
        ("timed".to_string(), 3000, Some("US".to_string()), Some("NY".to_string()), None, None, 4, 7),
    ];
    storage.upsert_analytics_batch(records).await.unwrap();
    ...
    let response = app
        .oneshot(
            Request::builder()
                .uri("/api/analytics/timed/aggregate?group_by=country&start_time=1500&end_time=2500")
                .header(header::AUTHORIZATION, "Bearer test-token")
                .body(Body::empty())
                .unwrap(),
        )
...
async fn test_analytics_aggregate_group_by_region() {
    ...
    let time_bucket = 1698768000;
    let records = vec![
        ("regions".to_string(), time_bucket, Some("US".to_string()), Some("CA".to_string()), Some("LA".to_string()), None, 4, 7),
        ("regions".to_string(), time_bucket, Some("US".to_string()), Some("NY".to_string()), Some("NYC".to_string()), None, 4, 4),
        ("regions".to_string(), time_bucket, Some("US".to_string()), Some("CA".to_string()), Some("SF".to_string()), None, 4, 2),
    ];
    storage.upsert_analytics_batch(records).await.unwrap();
