...
async fn test_analytics_aggregate_api_endpoint() {
    ...
    let response = app
        .oneshot(
            Request::builder()
                .uri(format!("/api/analytics/{}/aggregate?group_by=country", encoded_code("multi")))
                .header(header::AUTHORIZATION, "Bearer test-token")
                .body(Body::empty())
                .unwrap(),
        )
...
async fn test_analytics_aggregate_with_aggregator_realtime() {
    ...
    let response = app
        .oneshot(
            Request::builder()
                .uri(format!("/api/analytics/{}/aggregate?group_by=country", encoded_code("realtime")))
                .header(header::AUTHORIZATION, "Bearer test-token")
                .body(Body::empty())
                .unwrap(),
        )
...
async fn test_analytics_aggregate_with_unknown_pending_events() {
    ...
    let response = app
        .oneshot(
            Request::builder()
                .uri(format!("/api/analytics/{}/aggregate?group_by=country", encoded_code("pending")))
                .header(header::AUTHORIZATION, "Bearer test-token")
                .body(Body::empty())
                .unwrap(),
        )
...
async fn test_analytics_aggregate_with_time_range() {
    ...
    let response = app
        .oneshot(
            Request::builder()
                .uri(format!("/api/analytics/{}/aggregate?group_by=country&start_time=1500&end_time=2500", encoded_code("timed")))
                .header(header::AUTHORIZATION, "Bearer test-token")
                .body(Body::empty())
                .unwrap(),
        )
...
async fn test_analytics_aggregate_group_by_region() {
    ...
    let response = app
        .oneshot(
            Request::builder()
                .uri(format!("/api/analytics/{}/aggregate?group_by=region", encoded_code("regions")))
                .header(header::AUTHORIZATION, "Bearer test-token")
                .body(Body::empty())
                .unwrap(),
        )
